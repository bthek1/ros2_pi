...
using pimesh_frontend::bearing;
using pimesh_frontend::camera_step;
using pimesh_frontend::change_basis;
using pimesh_frontend::fit_rigid;
using pimesh_frontend::fit_rigid_robust;
using pimesh_frontend::fit_rotation_robust;
using pimesh_frontend::mean_residual_m;
using pimesh_frontend::RigidFit;
using pimesh_frontend::ScaleHandling;
using pimesh_frontend::sample_depth;
using pimesh_frontend::unproject;
...
TEST(ChangeBasis, ALeverArmTurnsARotationIntoATranslation)
{
  ...
  EXPECT_LT(
    cv::norm(
      same.rotation() -
      pimesh_frontend::change_basis(optical_basis(), spin.rotation())), 1e-12);
}
...
{
  const auto c = pimesh_frontend::unconstrained_covariance();
