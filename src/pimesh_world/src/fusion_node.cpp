...
#include "pimesh_world/fusion_node.hpp"
...
void FusionNode::process(Frame & frame)
{
  ...
  const cv::Mat colour = frame.rgb ? pimesh_perception::mat_over(*frame.rgb) : cv::Mat();
...
void FusionNode::log_stats()
{
  ...
  msg->latency_p95_ms = static_cast<float>(pimesh_perception::percentile(total, 0.95));
...
RCLCPP_COMPONENTS_REGISTER_NODE(pimesh_world::FusionNode)
