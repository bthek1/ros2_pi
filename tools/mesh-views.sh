...
#
# The renderer is a software rasteriser in numpy (tools/mesh_render.py): no GL, no
...
#
#   bash tools/mesh-views.sh <mesh.ply> [output-dir] [prefix]
#
...
MESH=${1:-}
[[ -n $MESH ]] || { echo "usage: bash tools/mesh-views.sh <mesh.ply> [out-dir] [prefix]"; exit 1; }
...
/usr/bin/python3 "$PIMESH_WS/tools/mesh_render.py" "$MESH" "$OUT_DIR" --prefix "$PREFIX"
