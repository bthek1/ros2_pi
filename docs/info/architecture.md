...
| `pimesh_core` | `ament_cmake` | both | **Split out by #14 P2, 2026-09-23.** Headers only, no library target: `mailbox.hpp` (newest-wins, one slot), `image_buffer.hpp` (a `cv::Mat` laid over a ROS image without copying it, bgr8 and 32FC1) and `stats.hpp` (the percentile and FNV-1a hash every probe reports its numbers through — four anonymous-namespace copies before it existed, one of which had the wrong offset basis since milestone A). **It exists because the include graph already said so**: `pimesh_world` was reaching into `pimesh_perception` for all three, and splitting depth out would have made that three consumers across three boundaries. `pimesh_camera` keeps its own copy on purpose — an edge from the Pi's package to a dev-box one would run the wrong way down the pipeline |
