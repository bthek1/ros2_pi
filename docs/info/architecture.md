...
| `pimesh_core` | `ament_cmake` | both | **Split out by #14 P2, 2026-09-23.** Headers only, no library target: `mailbox.hpp` (newest-wins, one slot), `image_buffer.hpp` (a `cv::Mat` laid over a ROS image without copying it, bgr8 and 32FC1) and `stats.hpp` (the percentile, the quartiles and the FNV-1a hash every probe reports its numbers through — four anonymous-namespace copies before it existed, one of which had the wrong offset basis since milestone A; `quartiles` was added for #10's P12 and shares `percentile`'s rank formula rather than restating it, with `test_stats` asserting the two agree at every sample count from 1 to 200). **It exists because the include graph already said so**: `pimesh_world` was reaching into `pimesh_perception` for all three, and splitting depth out would have made that three consumers across three boundaries. `pimesh_camera` keeps its own copy on purpose — an edge from the Pi's package to a dev-box one would run the wrong way down the pipeline |
| `pimesh_frontend` | `ament_cmake` | dev box (**builds on both**) | **Built 2026-09-12; depth split out into `pimesh_depth` by #14 P2 on 2026-09-23.** `decode_node` — the container's one network subscriber, JPEG → `bgr8`, **1.90 ms/frame**; `keypoint_node` — ORB 500 features, pooled matching, and each corner's position one frame earlier, **6.82 ms/frame** with depth and odometry beside it against an 8 ms budget; `odometry_node` — **the SLAM front end**, split out of `keypoint_node` on 2026-09-23 because that node had been publishing two `/pipeline/stats` rows since P7, being two stages wearing one name: a 6-DoF pose from `cv::solvePnPRansac` against the newest keyframe's landmarks at **1.37 px over 94 inliers, 81.6% of depth frames posed**, with `rotation_only` kept selectable as `gates/odom.sh`'s control; `ipc_probe` and `odom_probe`, the instruments `gates/ipc.sh` and `gates/odom.sh` measure with — the second reads the trajectory off `/odom` rather than off the node's own counters, because the failure P7 fixed was a pose every internal number described correctly and that left the node inverted; and `keypoints_view.hpp`, the conversion from a `Keypoints` message back into corners, ids, descriptors and one-frame-apart pairs — three loops inside `odometry_node`'s callback until 2026-09-23, two of which read past the end of a vector on a message whose parallel arrays disagreed |
| `pimesh_depth` | `ament_cmake` | dev box, **GPU** (**builds on both**) | **Built 2026-09-15 as part of `pimesh_perception`; its own package since #14 P2, 2026-09-23.** Also holds `scale_probe` and `depth_patch.hpp` since #10's P12 (2026-09-25) — the centred-patch median `tools/gates/scale.sh` compares against a tape measure, which is where this project's *unit* will come from; it excludes and counts far-clip pixels, because a clipped reading is the absence of a distance written as 6 m and depth is linear in `depth_scale` only below it. `depth_node` — Depth Anything V2 Small on the CUDA execution provider, **55.10 ms/frame, 17.4 Hz**, the pipeline's clock; `depth_probe`, the instrument `gates/depth.sh` measures with. **It builds on the Pi, where it can never run**, and that is why the inference engine is behind an interface: `depth_engine_ort.cpp` is compiled only where CMake finds ONNX Runtime, `depth_engine_null.cpp` everywhere else, and the node, its registration and its parameters are identical on both machines. Building it only where the GPU stack exists would weaken `test_transforms`'s ament-index check on the machine where it matters least, by weakening it everywhere |
| `pimesh_mapping` | `ament_cmake` | dev box | `fusion_node` (TSDF), `mesh_node` (marching cubes, PLY export) |
| `pimesh_dataset` | `ament_cmake` | dev box (**builds on both**) | **Built 2026-09-25** — #10's P11. `dataset_node` — a public RGB-D sequence where the camera usually is, publishing the dataset's own frames at the dataset's own stamps with the dataset's own intrinsics on `camera_node`'s two topics. **It stands in for the camera exactly**, `/pipeline/stats` row included, because a trajectory measured against TUM's ground truth only says something about the pipeline that runs on the Pi's camera if nothing downstream can tell which source is publishing. Loaded only with `source:=dataset_node`, never beside a camera or a bag: two publishers on `/image_raw/compressed` is the one-session failure seen from the producing end. It does not loop (a replay that restarts sends every stamp backwards and freezes the TF tree), does not re-encode (PNG goes on the wire as PNG; `cv::imdecode` reads either codec), and does not invent intrinsics — it reads a standard `camera_info` YAML through `pimesh_camera`'s own loader, handing it the size of the **first decoded frame**, so serving the C922's 1280×720 calibration over Freiburg's 640×480 frames is a refusal to start rather than an ATE that is really a measurement of the mismatch |
| `pimesh_instruments` | `ament_cmake` | both | **Split out by #14 P5, 2026-09-23.** No nodes and no library: the suites behind the Python instruments a gate's number comes out of — `test_straightness`, `test_mesh_render`, `test_orb_reference` |
...
**Every package builds from source on both machines**, and only `pimesh_camera`
and `pimesh_msgs` actually *run* on the Pi. The rest build there so that
`gates/build.sh` and `gates/test.sh` can assert the same sources compile and the
same suites pass at both ends — which is what catches a C++20 spelling or a
Lyrical-only header, on the machine where that is the only thing that would.
C++17 throughout, and APIs that exist in both distros — see
[CLAUDE.md](../../CLAUDE.md) on the ABI split.

## Topics

| Topic | Type | Publisher | QoS | Notes |
| --- | --- | --- | --- | --- |
| `/image_raw/compressed` | `sensor_msgs/CompressedImage` | `camera_node`, **or `dataset_node` with `source:=dataset_node`, or `ros2 bag play` — never two at once** | RELIABLE, KEEP_LAST(1) | **Live 2026-09-09.** The only topic on the LAN. MJPEG straight from V4L2, never re-encoded. ~80 kB/frame, measured 44–59 Hz on the dev box against 59 Hz at the Pi |
| `/camera_info` | `sensor_msgs/CameraInfo` | `camera_node` **or `dataset_node`** | RELIABLE, KEEP_LAST(1), transient local | **Live 2026-09-09, and carrying a real calibration since 2026-09-12** ([P9](https://github.com/bthek1/ros2_pi/issues/9), closed): fx=953.4, fy=957.6, cx=627.7, cy=334.6, held-out reprojection 0.4955 px, loaded from `package://pimesh_bringup/config/camera_info/c922_720p.yaml`. Falls back to *nominal* intrinsics with a startup WARNING if that file is absent |
...
- The TSDF volume is owned by `fusion_node` and never handed out as a message.
  ...
  **The two rendezvous through a process-local registry rather than a topic**
  (`pimesh_mapping/shared_volume.hpp`): the volume is over a gigabyte and its only
