...
| `pimesh_perception` | `ament_cmake` | dev box | **built** — `decode_node` (JPEG → bgr8, published zero-copy), `keypoint_node` (ORB + a rotation-only odometer that K-all-zeros keeps switched off), `depth_node` (ONNX Runtime on CUDA) and `ipc_probe_node`. `depth_node` is skipped by CMake when ONNX Runtime is absent, so a fresh clone still builds |
...
| `/depth` | `sensor_msgs/Image` (32FC1) | `depth_node` | RELIABLE, KEEP_LAST(1) | **live** — metres, bounded at `max_depth_m`, carrying the *input frame's* stamp and `camera_optical_frame`. The metres are **relative** until `depth_scale` is pinned at P5 |
| `/depth/rgb` | `sensor_msgs/Image` (bgr8) | `depth_node` | RELIABLE, KEEP_LAST(1) | **live** — the exact frame inferred on, republished at the same stamp, so fusion gets a true RGB-D pair with no sync guessing. Verified byte-identical by `just gate-depth`. A *separate* republisher cannot do this: it and depth drop different frames, so their stamp sets rarely intersect and an exact-time synchroniser limps at a couple of hertz |
| `/depth/image/compressed` | `sensor_msgs/CompressedImage` | `depth_node` | RELIABLE, KEEP_LAST(1) | **P8** — colourised preview for the dashboard. Not built at P4: nothing consumes it yet, and it would be drawn on its own thread like the keypoint preview. RELIABLE for the same reason as that one: a 720p JPEG fragments |
