...
`evo`: **Sim(3)-aligned ATE RMSE 0.27–0.36 m over six runs** of ~350 poses, 100% of them associated
...
| Truth | *(`evo`, offline)* | dev box | TUM fr1/desk, **ATE 0.27–0.36 m Sim(3)-aligned over six runs, RPE 0.14–0.15 m**, fitted scale 0.46–0.52 |
...
0.46–0.52 over six runs, so the sequence says `depth_scale` should be **4.6–5.2** against the
...
| [docs/info/testing.md](docs/info/testing.md) | The 459 unit tests: what each suite exists to catch, and why a wrong version of that code would otherwise look fine |
