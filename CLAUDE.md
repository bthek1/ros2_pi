...
**2026-09-23** nine packages build from source under both distros and a webcam on
...
**And as of 2026-09-25 there is one number about the pose that this project did
not produce.** P11 replays TUM RGB-D fr1/desk — 613 frames with a 100 Hz
motion-capture trajectory — through the real container and hands the result to
`evo`: **Sim(3)-aligned ATE RMSE 0.27–0.36 m over seven runs** of ~350 poses, 100% of them associated
against the truth, RPE **0.15 m** over a 1 s window. Every figure P0–P10 asserts
about the trajectory is internal, and P7 is what that is worth: the rotation was
composed inverted for six days and every internal number describing it was right.
...
| Capture (eval) | `dataset_node` | dev box | TUM fr1/desk at its own stamps and intrinsics, **613 of 613 frames, 0 behind schedule** |
...
| Truth | *(`evo`, offline)* | dev box | TUM fr1/desk, **ATE 0.27–0.36 m Sim(3)-aligned over seven runs, RPE 0.14–0.15 m**, fitted scale 0.46–0.52 |
...
and is only discovered at the far end of an `rsync`. `bash tools/pi/build-pi.sh` is cheap —
...
- **A looping bag and a node publishing TF from it cannot both be right**,
  ...
  **`--clock` with `use_sim_time` is the obvious fix and is measured wrong.** The
  ...
  So: `tools/view/replay.sh` loops and starts `pimesh.launch.py pipeline:=false` —
  the static frame tree, no components — and `tools/view/view-keypoints.sh` needs the
...
- **One session at a time, and this is the same rule as "one Wi-Fi reader" seen
  ...
  `assert_no_session` in `tools/lib/just-lib.sh` is the guard, and **every script
...
- **Reset the camera before measuring anything about it.**
  `bash tools/calib/camera-reset.sh` puts every control back to its default, forces
...
- **`/camera_info` comes from a file, and there is no `calibrated` flag.**
  ...
  `bash tools/calib/calibrate.sh`, check it with `bash tools/gates/calibration.sh`.
...
- **Calibration frames come from a bag, chosen offline.**
  `bash tools/calib/calibrate.sh record` then `select` — the live `grab` is the quick-look
...
- **Two board conventions, one apart, and our own printed sheet gets it wrong.**
  ...
  be edited, so do not copy the command off it — `bash tools/calib/calibrate.sh` takes
...
- **`findChessboardCorners` can lock onto a lattice one square out, and only the
  ...
  (`confirm_grid` in `tools/calib/calib_straightness.py`), in the grabber *and* in the gate.
...
- **`cv2.aruco` constructors that exist on both machines and mean different things —
  ...
  version-branched in `tools/calib/calib_straightness.py` (`>= 4.8` and `>= 4.7`
...
- **A board photographed only square-on calibrates to nonsense, and every check
  ...
  why `tools/calib/calibrate.sh` grabs its frames before the calibration session** — an
...
- **A loop bounded by one array's length while indexing a different one, in a
  ...
  **A struct-of-arrays message is a ragged-array hazard by construction.** The
  ...
  a crash.
- **A test can enforce a contract the compiler was already enforcing, and the
  test then reads as the thing keeping it.** `Percentile.DoesNotDisturbTheCallersVector`
  said in its own comment that it was "the test that keeps" `percentile`'s
  by-value signature. Measured 2026-09-25: changing that parameter to a reference
  does not fail the test, it fails to **compile**, at five other cases in the same
  file that pass a temporary — and a reference *overload* is ambiguous at the same
  places. So the property held, by an accident of which cases happen to exist,
  which any refactor could have removed silently. Two lessons and the second is
  the general one: a signature contract belongs in a **`static_assert` carrying
  its own message**, where breaking it names itself; and **a comment claiming
  which check holds a property is a claim to verify like any other** — this is the
  `nav_msgs` covariance citation again, where a code comment was read instead of
  the message definition. Found by mutation, which is the only thing that finds
  it: the suite is green either way.
...
- **Two things that have to hold the same value across a *language* boundary are
  ...
  **A fifth pair joined them on 2026-09-25, across a *shell* boundary this time.**
  The `depth_scale_reference` comment that records P12's tape measure is parsed by
  `tools/gates/scale.sh` and validated by `test_transforms`, as two regexes that
  agreed only because one person typed both. The looser-test direction is the bad
  one: it accepts a marker the gate cannot find, and the gate then refuses with
  "no measured distance recorded" about a line plainly in the file.
  `test_transforms` now reads the regex out of the shell script and asserts it is
  the same string — the same method, applied to a second pair. It was found by
  mutating the marker, not by reading the code.
...
- **A gate's instrument needs its own tests, and the reference implementation is
  ...
  matched-keypoint fraction is within 5 points of `tools/eval/orb_reference.py` over
...
- **This repo is the colcon workspace.** Packages go in `src/`, named
  `pimesh_<thing>` — nine exist: `pimesh_msgs`, `pimesh_bringup`,
  ...
  are `pimesh_perception` split in two and `pimesh_world` renamed, plus
  `pimesh_instruments` (#14's P5) and — since #10's P11 on 2026-09-25 —
  `pimesh_dataset`, whose one component stands in for the camera. **The prefix
  ...
  **`pimesh_depth` builds on the Pi too, and the GPU code
  ...
  matters least, by weakening it everywhere.

  **`pimesh_dataset` is the one package that depends on `pimesh_camera`, and the
  direction is the whole justification.** It reads a standard `camera_info` YAML
  through `pimesh_camera::load_calibration` rather than carrying a copy —
  including that loader's refusal on a width/height mismatch, which is what stops
  the C922's intrinsics being served over a 640×480 dataset. The rule above is
  that an edge from the *Pi's* package to a dev-box one points the wrong way down
  the pipeline; this one runs the other way, and `dataset_node` is the same
  *stage* as `camera_node` besides, so two implementations of "what does this YAML
  mean" is exactly the duplication that ends in them disagreeing.
  (Not `ros2_pi_*`: a `ros2_` prefix reads as core tooling.)
  Shared shell helpers live in `tools/` and are rsynced to the Pi, so they must
  work on both distros — `tools/lib/ros-env.sh` discovers the distro rather than
  naming it, and `tools/pi/check-stale.sh` is run by both `gates/build.sh` here and
  `sync-pi` there.
...
- **Nodes are `rclcpp_components` components**, registered with
  ...
  before P2, and it exists again because `tools/view/replay.sh` must not publish a
...
- **Parameters live in `config/*.yaml`, keyed by node name**, and launch files in
  ...
  **And once where the two halves are a number and its justification, in one
  file, related by nothing but adjacency.** `depth_scale` is the constant under
  every distance this project reports, and what makes it a measurement rather
  than a guess is the tape figure recorded beside it — as a **comment**, which
  `tools/gates/scale.sh` parses:
  ...
  A parsed comment is an odd contract and it is the right one here. A second
  file, or a parameter no node declares, both put the justification somewhere the
  number can drift away from — which is the `volume_key` trap with the halves in
  different places. **Adjacency is the guarantee**, so
  `test_the_depth_scale_reference_marker_is_well_formed_if_it_exists` asserts
  there is at most one marker and that it is the line immediately above
  `depth_scale`. Absent is a pass in that test, because P12 has not happened and a
  test that fails until somebody goes to a room is one people learn to ignore; the
  *refusal* lives in the gate, where a missing measurement has to stop something.
  **The first version of that test reported two markers in a file that has none** —
  it matched the prose describing the marker and the example beside it, so the
  pattern requires the token to be the first thing after the `#`.
...
- **Build with `just build`**, not bare `colcon`. The recipe is
  ...
  within 2% of `-O3` and leaves a node you can put a debugger on. `bash tools/pi/build-pi.sh` does the same over
  SSH after `bash tools/pi/sync-pi.sh` ships source — source only, never a built tree.
...
- **The justfile is the user-facing surface — everything else is a script.**
  It holds `build` and the six viewers — `view-camera`, `replay`,
  `view-keypoints`, `view-depth`, `view-mesh`, `view-odom` and `dashboard` — and
  that is the whole of it: what someone types on a normal day. (`hello-compose`
  and `hello-lan` went with `pimesh_hello` on 2026-09-23.) Gates, the Pi plumbing, the straggler
...
- **A `{{ }}` substitution in a recipe body is text, not an argument, and an
  ...
  **What let it ship is the more useful half.** `gates/hello-clean.sh` signals
  both recipes, five endings each — and it invokes `tools/view/view-odom.sh`
...
- **The shell lives in `tools/`.** Every recipe is one line that runs a script,
  ...
  `tools/lib/just-lib.sh`, which every script sources; `tools/` is rsynced, so the
  same functions work at both ends.

  **`tools/` is grouped as of #14's P6**, and where a script goes says what kind
  of thing it is: `gates/` closes a phase, `view/` is for a person to look at,
  `pi/` reaches the far machine, `calib/` is the calibration session and its
  helpers, `eval/` holds the Python instruments a gate's number comes out of, and
  `lib/` is sourced rather than run. What stays at the top is what a person runs
  from the workspace root — `build.sh`, `test.sh`, `clean.sh`, `stragglers.sh`,
  `record-clip.sh` and the two fetchers. `gates/` did not move, because
  `bash tools/gates/<name>.sh` is written into this file, the README, the
  justfile's header and twelve issue bodies. The reason is not tidiness: `just` gives a
...
- **Sessions tear themselves down — no stragglers.** Ctrl-C, a closed terminal
  ...
  `arm_cleanup` in `tools/lib/just-lib.sh` installing a handler on EXIT and on
  ...
  **`run_for`** (`timeout --foreground -s INT`, in `tools/lib/just-lib.sh`) for
...
| [docs/info/testing.md](docs/info/testing.md) | The 478 unit tests: what each suite exists to catch, and why a wrong version of that code would otherwise look fine |
...
| [#10](https://github.com/bthek1/ros2_pi/issues/10) [#11](https://github.com/bthek1/ros2_pi/issues/11) [#12](https://github.com/bthek1/ros2_pi/issues/12) [#13](https://github.com/bthek1/ros2_pi/issues/13) **(all open, none started)** — milestones F–I | **Turning the pipeline into monocular visual SLAM.** Phases P11–P20 on the same one list, four contiguous slices. **F (P11–P13) is the only one not labelled `deferred`, and it is first for a reason that is this file's whole argument**: nothing in P0–P10 ever measured the pose against a truth, so P11 is a dataset source, a TUM-format trajectory and an ATE through `evo`, and P12–P13 are the visit to the room that everything else has been waiting on — a tape measure for `depth_scale`, and `bags/walk1`. **P11 is done, 2026-09-25**: `bash tools/gates/trajectory.sh`, ATE 0.27–0.36 m Sim(3)-aligned on TUM fr1/desk over seven runs, and a fitted scale that bounds `depth_scale` at 4.6–5.2 without a tape measure. **P12 and P13 are what is left of F, and both need a person in the room.** Then **G (P14–P15)**: `MapPoint`s observed by many keyframes and local bundle adjustment, which is the noun this pipeline does not have — a landmark currently dies with the keyframe that saw it, and `rgbd_odometry` poses each frame against the **newest keyframe only**. Then **H (P16–P18)**: place recognition against the whole store, a pose graph that finally publishes `map -> odom` for real, and a TSDF **rebuilt** at the corrected poses — without that last one a closure fixes the trajectory and leaves the room where it was. Then **I (P19–P20)**: a `LOST` state that stops fusing, and relocalisation. **One dependency was checked before any of it was filed**: g2o ships with ROS at both ends — `ros-lyrical-libg2o` and `ros-jazzy-libg2o`, both 2020.5.29, both exporting `g2o::core`, `g2o::types_sba`, `g2o::types_sim3` and `g2o::solver_eigen` under identical names — so the cross-distro tax here is **zero** and no `depth_engine_null.cpp`-style conditional engine is needed. **DBoW2 is in neither apt**, so P16 starts brute force with the trigger written down |
| `bash tools/gates/naming.sh` | **#14's guard, and the only gate here whose subject is the tree rather than the pipeline.** Five checks, each covering a rename that a C++ compiler has no opinion about. (a) every `RCLCPP_COMPONENTS_REGISTER_NODE` is named by a launch list **and its package field agrees with the plugin's namespace** — the container resolves the class in the index of the package the *second* field names, so a right plugin string beside a wrong package builds perfectly and fails at launch. (b) every `PIMESH_*_PAT` in `just-lib.sh` matches a command line **synthesised from the tree and from `/opt/ros`**, never a typed literal — a kill pattern that matches nothing makes `stragglers.sh` print 0 over a leaked `camera_node` holding `/dev/video0`. (c) every path and `pimesh_*` name in the documents that tell you what to run resolves — **and in the scripts too**, since a script naming a moved script is the same failure with no reader to notice it. (d) every library header outside `nodes/` neither includes `rclcpp/` nor goes unnamed by a test, which is the `percentile`/`depth_mat_over`/`quote` lesson made mechanical. (e) `apps/` is files with a `main()`, `probes/` is components a gate loads, and **nothing in `src/` has a `main()`**. Both exemption lists assert they are still *needed*: the moment an exempted path exists or an exempted header gets a test, the gate says to delete the line |
| `bash tools/gates/ipc.sh` / `bash tools/gates/keypoints.sh` | **P2 and P3's gates.** `ipc.sh` runs the real container twice against the Pi's live camera and compares published buffer addresses with intra-process comms on and off — and asserts the decoded topic has at least two subscribers, because one consumer is the configuration that cannot fail. `keypoints.sh` replays `bags/desk1` and measures three things three ways: the rate from a C++ subscriber's steady clock, the per-frame cost from the node's own log line, and the matched-keypoint fraction against `tools/eval/orb_reference.py` — the predecessor's algorithm reimplemented in Python over the same clip, which is the only part of the gate with an outside opinion about whether the corners mean anything |
...
| `bash tools/gates/trajectory.sh` | **P11's gate, and the only number in this project about the pose that this project did not produce.** It replays TUM fr1/desk through the real container — `dataset_node` composed in as the capture stage, `odom_probe` writing a TUM-format trajectory — and hands the result to `evo`, because writing our own ATE would be writing the instrument and the thing it measures in the same afternoon. **Two ATEs are printed and one is asserted**: SE(3)-aligned is very largely a measurement of `depth_scale`, so the Sim(3) figure — the trajectory's *shape* — is the one under the ceiling. The **fitted scale is worth more than the assertion**, because on a metric dataset it is `depth_scale`'s answer without a tape measure: 0.455–0.517 over seven runs, so 4.6–5.2 against the 10.0 in the YAML. **Three controls, not one.** `rotation_only` must not reach the ceiling, and it does not reach it at all — with translation identically zero the covariance is rank-deficient and Umeyama refuses to align it, so the gate also asserts the ceiling is below the **0.8559 m an estimate that never moves would score**, derived from `groundtruth.txt` rather than typed. `dataset_node` started with the **C922's calibration** over 640×480 frames must refuse to start. And the intrinsics are read **off the wire**, not off the node's own log. **The finding it produced is not in any of that**: the written trajectory is the *camera optical* pose, not `base_link`'s — rotating TUM's own ground truth by that constant scores **0.654 m RPE against itself**, which is essentially the whole of the 0.768 m the first run reported. An ATE cannot see it; that is why RPE is reported |
| `bash tools/gates/scale.sh` | **P12's gate, written 2026-09-25 and waiting on a person.** It replays `bags/scale1` — a flat surface at a tape-measured distance — and asserts the median of `/depth` over a *centred patch* agrees with the tape within 3%. **A patch and not the pixel at the principal point**, which is the false green P12 names by hand: a depth map carries z and not ray length, so the principal point is the one place those agree exactly, and a single reading there would let one hot pixel set this project's unit for good. It **refuses rather than passes** in four places — no clip, no tape figure recorded, the file and the command line disagreeing, and a patch that is mostly far clip — because a vacuous pass would put a made-up unit under every later measurement here. The clip refusal is the sharp one: `depth_to_metres` writes exactly `max_range_m` where the model's inverse depth falls below its floor, so a clipped pixel is the *absence* of a distance dressed as 6 m, and depth is linear in `depth_scale` only below it — an implied scale over a clipped patch comes out **too small** and looks fine. **Run against `bags/desk1` standing in for the clip, it refuses at 0.30 of the patch clipped and a within-frame spread of 0.19**, so that budget has been watched to exclude something. On the first run at the wall it prints the two lines to paste into `config/pimesh.yaml` — **unless a check failed, in which case it prints no number at all**, because handing somebody a scale it has just called wrong is the failure this whole gate is about |
| `bash tools/fetch-dataset.sh` | **P11's data, with three hashes rather than one.** TUM fr1/desk into `~/.local/share/pimesh-datasets`, no sudo, outside the workspace because it is not this project's data. The tarball's sha256 says the download is what TUM published, `rgb.txt`'s says the frame list has not moved, and **`groundtruth.txt`'s says the truth has not** — the one that would otherwise be unfalsifiable, since a corrupted truth file does not produce an error, it produces an ATE. Verified on every run, not only after a fetch; a tampered tree is named and re-extracted |
